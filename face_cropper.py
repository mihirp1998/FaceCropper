...
def _main(parent_dir_path,cnn_path,save_dir_path):
	...
	for child_dir in os.listdir(parent_dir_path):
		...
		if os.path.isdir("newCrop/" + label):
			continue 
		label_count += 1
		...
		for image_name in os.listdir(parent_dir_path+child_dir):
			...
			try:
				...
				for k, d in enumerate(dets):
					...
					marginV1 = marginV
					marginH = int(abs(left - right) *0.5)
					marginH1 = marginH
					...
					save_dir = save_dir_path+label
					if not os.path.exists(save_dir):
						os.mkdir(save_dir)

					cv2.imwrite(save_dir+'/'+image_name.replace(":","_"), cropped_img)
